...
metrics = AsconMetrics("server_logistic_regression")
app = FastAPI(title="server_hfl_logistic_regression")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_methods=["*"],
...
<head>
    ...
    <title>server_hfl_logistic_regression</title>
    <style>
        :root {
            --bg: #0b1220;
            --panel: #121a2b;
...
<body>
    <div class="wrap">
        <section class="hero">
            <h1>Servidor HFL - Logistic Regression</h1>
        ...
        <section class="grid">
            <article class="card">
                ...
                <div class="value" id="service-name">server_hfl_logistic_regression</div>
            </article>
            <article class="card">
                <div class="label">Modelo</div>
                <div class="value" id="model-name">Logistic Regression</div>
            </article>
            <article class="card">
    ...
    <script>
            ...
            const rows = records.slice().reverse().map((record) => {
                return `
                    <tr>
                        ...
                        <td>${escapeHtml(record.model ?? 'Logistic Regression')}</td>
                    </tr>
                `;
            }).join('');
        ...
        async function refresh() {
            try {
                ...
                document.getElementById('model-name').textContent = status.model ?? 'Logistic Regression';

...
- Intentos analizados: `9`
- Rango de intentos: `3` a `11`
- p95 promedio ESP32->RPi decrypt: `4.680 ms`
- p95 promedio RPi->PC decrypt: `20.350 ms`
- Overhead promedio de payload: `1044.407 bytes`
- Accuracy local promedio: `0.9318`
- Loss local promedio: `0.1907`
- Accuracy global final promedio: `0.9463`
- Loss global final promedio: `0.1415`
- Completion rate promedio: `0.7037`

## Metric Catalog

| metric_name | exactness | source | aggregation | purpose |
| --- | --- | --- | --- | --- |
| transport.edge_rpi.decrypt.p95_ms | exact_from_csv | ascon_metrics_gateway_* | p95(elapsed_ms) WHERE channel='ESP32->RPi' AND operation='decrypt' | SLI de latencia de ingreso al gateway por muestra. |
...
| transport.payload.overhead.avg_bytes | exact_from_csv | ascon_metrics_gateway_*, ascon_metrics_server_* | avg(overhead_bytes) | Costo promedio en bytes introducido por el envelope de ASCON. |
| transport.payload.expansion.avg_ratio | exact_from_csv | ascon_metrics_gateway_*, ascon_metrics_server_* | avg(enc_bytes / pt_bytes) | Factor de expansion promedio del payload transmitido. |
...
| --- | --- | --- | --- | --- |
| transport.crypto.gateway | reconstructed_from_csv | ascon_metrics_gateway_* | timestamp_dt, attempt_id, gateway_id, channel, operation, elapsed_ms, pt_bytes, enc_bytes, overhead_bytes, client_id, sample_label_name | Log normalizado derivado sin tocar el CSV original. |
| transport.crypto.server | reconstructed_from_csv | ascon_metrics_server_* | timestamp_dt, attempt_id, channel, operation, elapsed_ms, pt_bytes, enc_bytes, overhead_bytes, round_ref | No contiene gateway_id explicito en los datos actuales. |
...
| model.deploy.gateway | live_stdout_only | gateway_hfl.py | trace_id, ts, gateway_id, round_ref, payload_bytes, apply_ms, status | Hoy solo puede estimarse desde transport encrypt del servidor. |

## Trace Catalog

| trace_name | status | trace_id_pattern | spans | notes |
| --- | --- | --- | --- | --- |
| round_trace | reconstructed_from_csv | attempt:{attempt_id}:round:{round_ref} | gateway_A.local_train, gateway_B.local_train, server.decrypt_batch, server.encrypt_global, global_round_commit | Traza exacta a nivel de ronda basada en CSV ya capturados. |
| gateway_round_trace | reconstructed_from_csv | attempt:{attempt_id}:round:{round_ref}:gateway:{gateway_id} | esp32_to_rpi_decrypt_batch, local_train_done | Traza util para diagnosticar carga y skew por gateway. |
...
| fedavg_trace | live_stdout_only | attempt:{attempt_id}:round:{round_ref}:fedavg | server.wait_updates, server.fedavg_compute, server.encrypt_global, server.deploy | Requiere logging efimero en tiempo real si se quiere exactitud completa. |
...
| Transport | Latency p95 by channel | bar | ascon_metrics_* | transport.*.p95_ms |
| Transport | Payload overhead by channel | bar | ascon_metrics_* | transport.payload.overhead.avg_bytes |
...
| Data Quality | Class mix by gateway | stacked_bar | ascon_metrics_gateway_* | data.class_mix.share |

"""
Herramientas de analisis SRE para hfl_v7-RN usando los CSV historicos ya
...
def _load_attempt_frames(attempt_id: int, attempt_dir: Path) -> dict[str, pd.DataFrame]:
    gateway_transport_paths = sorted(attempt_dir.glob("ascon_metrics_gateway_*.csv"))
    ...
    if not gateway_transport_df.empty:
        ...
        gateway_transport_df["event_type"] = "transport.crypto"
        ...
        inventory_rows.append(
            {
                ...
                "gateway_transport_files": int(len(list(attempt_dir.glob("ascon_metrics_gateway_*.csv")))),
...
def build_metric_catalog() -> pd.DataFrame:
    rows = [
        ...
        },
        {
            ...
            "exactness": "exact_from_csv",
            "source": "ascon_metrics_gateway_*, ascon_metrics_server_*",
...
def build_log_catalog() -> pd.DataFrame:
    rows = [
        ...
        },
        {
            ...
            "notes": "Hoy solo puede estimarse desde transport encrypt del servidor.",
        },
    ]
    return pd.DataFrame(rows)
    ...
    rows = [
        {
            ...
            "spans": "gateway_A.local_train, gateway_B.local_train, server.decrypt_batch, server.encrypt_global, global_round_commit",
        ...
        },
        {
            ...
            "spans": "esp32_to_rpi_decrypt_batch, local_train_done",
        ...
        },
        {
            ...
            "spans": "server.wait_updates, server.fedavg_compute, server.encrypt_global, server.deploy",
...
def generate_plots(
    ...
    if not transport_summary.empty:
        ...
        ax.set_title("Latency p95 promedio por canal y operacion (Intentos #3-#11)")
        ax.set_xlabel("Canal")
    ...
    if not global_history_enriched.empty:
        ...
        ax1.set_title("Convergencia global promedio (#3-#11)")
...
def write_spec_markdown(
    ...
    lines = [
        "# SRE Observability Spec - hfl_v7-RN",
        "",
        "Generado a partir de `Results/#3` a `#11` sin modificar los CSV fuente.",
        "",
        "## Executive Summary",
        "",
...
if __name__ == "__main__":
    default_results_root = Path(
        r"C:\Users\VivoBook\Downloads\microproyecto2\microproyecto 2 porque wtf\src\hfl_v7-RN\Results"
    )
    default_output_root = Path(
        r"C:\Users\VivoBook\Downloads\microproyecto2\microproyecto 2 porque wtf\src\Analisis de Modelos\RN\analysis_outputs"
    )
    bundle = run_analysis(

"""
Herramientas de analisis SRE para hfl_v7-CNN usando los CSV historicos ya
guardados en Results_FOG/#N. No modifica los archivos fuente del experimento.
"""
...
class AnalysisConfig:
    ...
    experiment_name: str = "hfl_v7-CNN_FOG"
...
def _load_attempt_frames(attempt_id: int, attempt_dir: Path) -> dict[str, pd.DataFrame]:
    gateway_transport_paths = sorted(
        path for path in attempt_dir.glob("ascon_metrics_*.csv") if "server" not in path.name.lower()
    )
    server_transport_path = _first_existing(attempt_dir, "ascon_metrics_server_*.csv")
    ...
    if not gateway_transport_df.empty:
        gateway_transport_df["channel"] = gateway_transport_df["channel"].replace(
            {
        ...
        gateway_transport_df["event_type"] = "transport.crypto.fog"
    ...
    if not server_transport.empty:
        server_transport["channel"] = server_transport["channel"].replace(
            {
                "RPi_leader->PC": "RPi->PC",
                "PC->RPi_leader": "PC->RPi",
            }
        ...
        server_transport["event_type"] = "transport.crypto.fog"
    ...
    if not model_metrics_df.empty:
        model_metrics_df["event_type"] = np.where(
            model_metrics_df["stage"].astype(str) == "fog_fedavg",
            "model.fog_fedavg",
            "model.local_train",
        )
...
def load_all_data(config: AnalysisConfig) -> dict[str, pd.DataFrame]:
    ...
    for attempt_id, attempt_dir in _discover_attempt_dirs(config):
        ...
        if not frames["gateway_transport"].empty:
            gateway_transport_parts.append(frames["gateway_transport"])
        if not frames["server_transport"].empty:
        ...
        inventory_rows.append(
            {
                ...
                "gateway_transport_files": int(
                    len([path for path in attempt_dir.glob("ascon_metrics_*.csv") if "server" not in path.name.lower()])
                ),
                "server_transport_files": int(len(list(attempt_dir.glob("ascon_metrics_server_*.csv")))),
...
def build_canonical_log_events(data: dict[str, pd.DataFrame]) -> pd.DataFrame:
    ...
    if not gateway_transport.empty:
        gateway_transport["event_name"] = "transport.crypto.gateway"
    ...
    if not server_transport.empty:
        server_transport["event_name"] = "transport.crypto.server"
    ...
    if not model_metrics.empty:
        model_metrics["event_name"] = np.where(
            model_metrics["stage"].astype(str) == "fog_fedavg",
            "model.fog_fedavg",
            "model.local_train",
        )
...
def build_metric_catalog() -> pd.DataFrame:
    rows = [
        {
            "metric_name": "transport.edge_rpi.decrypt.p95_ms",
            "exactness": "exact_from_csv",
            "source": "ascon_metrics_CNN_FOG_gateway_*",
            "aggregation": "p95(elapsed_ms) WHERE channel='ESP32->RPi' AND operation='decrypt'",
            "purpose": "SLI de latencia de ingreso al gateway por muestra.",
        },
        {
            "metric_name": "transport.rpi_pc.decrypt.p95_ms",
            "exactness": "exact_from_csv",
            "source": "ascon_metrics_server_*",
            "aggregation": "p95(elapsed_ms) WHERE channel='RPi->PC' AND operation='decrypt'",
            "purpose": "SLI de latencia de descifrado en servidor para actualizaciones de gateways.",
        },
        {
            "metric_name": "transport.pc_rpi.encrypt.p95_ms",
            "exactness": "exact_from_csv",
            "source": "ascon_metrics_server_*",
            "aggregation": "p95(elapsed_ms) WHERE channel='PC->RPi' AND operation='encrypt'",
            "purpose": "SLI de despliegue del modelo global desde el servidor.",
        },
        {
            "metric_name": "transport.payload.overhead.avg_bytes",
            "exactness": "exact_from_csv",
            "source": "ascon_metrics_CNN_FOG_gateway_*, ascon_metrics_server_fog_*",
            "aggregation": "avg(overhead_bytes)",
            "purpose": "Costo promedio en bytes introducido por el envelope de ASCON.",
        },
        {
            "metric_name": "transport.payload.expansion.avg_ratio",
            "exactness": "exact_from_csv",
            "source": "ascon_metrics_CNN_FOG_gateway_*, ascon_metrics_server_fog_*",
            "aggregation": "avg(enc_bytes / pt_bytes)",
            "purpose": "Factor de expansion promedio del payload transmitido.",
        },
        {
        ...
        {
            "metric_name": "fog.preaggregation.accuracy.avg",
            "exactness": "exact_from_csv",
            "source": "model_metrics_gateway_*",
            "aggregation": "avg(accuracy) WHERE stage='fog_fedavg'",
            "purpose": "Calidad media del pre-FedAvg realizado por el leader Fog.",
        },
        {
            "metric_name": "fog.preaggregation.loss.avg",
            "exactness": "exact_from_csv",
            "source": "model_metrics_gateway_*",
            "aggregation": "avg(loss) WHERE stage='fog_fedavg'",
            "purpose": "Estabilidad media del pre-FedAvg realizado por el leader Fog.",
        },
        {
...
def build_log_catalog() -> pd.DataFrame:
    rows = [
        {
            "log_event": "transport.crypto.gateway",
            "status": "reconstructed_from_csv",
            "source": "ascon_metrics_CNN_FOG_gateway_*",
            "fields": "timestamp_dt, attempt_id, gateway_id, channel, operation, elapsed_ms, pt_bytes, enc_bytes, overhead_bytes, client_id, sample_label_name",
            "notes": "Incluye trafico ESP32<->RPi y trafico Fog RPi_peer<->RPi_leader sin tocar el CSV original.",
        },
        {
            "log_event": "transport.crypto.server",
            "status": "reconstructed_from_csv",
            "source": "ascon_metrics_server_fog_*",
            "fields": "timestamp_dt, attempt_id, channel, operation, elapsed_ms, pt_bytes, enc_bytes, overhead_bytes, round_ref",
            "notes": "Representa el intercambio cifrado entre leader Fog y PC. No contiene gateway_id explicito.",
        },
        {
        ...
        {
            "log_event": "model.fog_fedavg",
            "status": "reconstructed_from_csv",
            "source": "model_metrics_gateway_*",
            "fields": "timestamp_dt, attempt_id, gateway_id, fl_round, round_ref, num_samples, accuracy, loss, fog_role, peer_count",
            "notes": "Representa el pre-FedAvg ejecutado por el leader Fog antes de enviar al servidor.",
        },
        {
            "log_event": "model.global_round",
            "status": "reconstructed_from_csv",
            "source": "global_weights_history_*",
            "fields": "timestamp_dt, attempt_id, round_ref, accuracy, loss, w3_mag, w4_normal, w4_brute, w4_scan, fog_samples",
            "notes": "Evento canonico de cierre de ronda global agregada desde Fog.",
        },
        {
            "log_event": "fedavg.compute",
            "status": "live_stdout_only",
            "source": "server_hfl_fog.py",
            "fields": "trace_id, ts_start, ts_end, round_ref, fog_clusters_received, fedavg_ms",
            "notes": "No reconstruible con CSV actuales; recomendado solo a consola JSON del servidor Fog.",
        },
        {
            "log_event": "model.deploy.gateway",
            "status": "live_stdout_only",
            "source": "gateway_hfl_fog.py",
            "fields": "trace_id, ts, gateway_id, round_ref, payload_bytes, apply_ms, status",
            "notes": "Hoy solo puede estimarse desde los eventos de cifrado del leader Fog y del servidor.",
        },
    ]
    return pd.DataFrame(rows)
    ...
    rows = [
        {
            ...
            "spans": "gateway_A.local_train, gateway_B.local_train, leader.fog_fedavg, server.decrypt_batch, server.encrypt_global, global_round_commit",
            "notes": "Traza exacta a nivel de ronda Fog basada en CSV ya capturados.",
        },
        {
            "trace_name": "gateway_round_trace",
            "status": "reconstructed_from_csv",
            "trace_id_pattern": "attempt:{attempt_id}:round:{round_ref}:gateway:{gateway_id}",
            "spans": "esp32_to_rpi_decrypt_batch, local_train_done, rpi_peer_to_leader_exchange",
            "notes": "Traza util para diagnosticar carga, skew y trafico intra-Fog por gateway.",
        },
        {
            "trace_name": "sample_trace",
            "status": "live_stdout_only",
            "trace_id_pattern": "attempt:{attempt_id}:round:{round_ref}:sample:{client_id}:{sample_seq}",
            "spans": "esp32.publish, gateway.decrypt, gateway.buffer, gateway.train_enqueue",
        ...
        },
        {
            ...
            "spans": "leader.wait_peer, leader.fog_fedavg, server.wait_fog_clusters, server.fedavg_compute, server.encrypt_global, server.deploy",
            "notes": "Requiere logging efimero en tiempo real si se quiere exactitud completa de la jerarquia Fog.",
        },
    ]
    return pd.DataFrame(rows)
...
def build_dashboard_catalog() -> pd.DataFrame:
    rows = [
        ...
        },
        {
            ...
            "chart_type": "bar",
            "source": "ascon_metrics_CNN_FOG_*",
            "metric": "transport.*.p95_ms",
        },
        {
            "panel_group": "Transport",
            "panel_name": "Payload overhead by channel",
            "chart_type": "bar",
            "source": "ascon_metrics_CNN_FOG_*",
            "metric": "transport.payload.overhead.avg_bytes",
        },
        {
        ...
        {
            "panel_group": "Fog Layer",
            "panel_name": "Leader fog_fedavg accuracy and loss",
            "chart_type": "line",
            "source": "model_metrics_gateway_*",
            "metric": "fog.preaggregation.*",
        },
        {
...
def compute_transport_summary(transport_events: pd.DataFrame) -> pd.DataFrame:
    ...
    df["expansion_ratio"] = pd.to_numeric(df["enc_bytes"], errors="coerce") / pd.to_numeric(df["pt_bytes"], errors="coerce")
    df["overhead_ratio_pct"] = pd.to_numeric(df["overhead_bytes"], errors="coerce") / pd.to_numeric(df["pt_bytes"], errors="coerce") * 100.0
            ...
            lambda group: pd.Series(
                {
                    ...
                    "avg_overhead_ratio_pct": _safe_mean(group["overhead_ratio_pct"]),
...
def compute_fog_aggregation_summary(model_metrics: pd.DataFrame) -> pd.DataFrame:
    if model_metrics.empty:
        return pd.DataFrame()

    df = model_metrics.loc[model_metrics["stage"] == "fog_fedavg"].copy()
    if df.empty:
        return pd.DataFrame()
            ...
            lambda group: pd.Series(
                {
                    ...
                    "std_loss": _safe_std(group["loss"]),
                    "avg_num_samples": _safe_mean(group["num_samples"]),
                    "avg_peer_count": _safe_mean(group["peer_count"]),
                }
            )
        )
        .reset_index()
...
def build_round_trace_summary(
    ...
    local_df = model_metrics.loc[model_metrics["stage"] == "local_train"].copy()
    local_df["global_round"] = pd.to_numeric(local_df["fl_round"], errors="coerce") + 1
    fog_df = model_metrics.loc[model_metrics["stage"] == "fog_fedavg"].copy()
    fog_df["global_round"] = pd.to_numeric(fog_df["fl_round"], errors="coerce") + 1
            ...
            lambda group: pd.Series(
                {
                    ...
                    "gateway_decrypt_avg_ms": _safe_mean(group["elapsed_ms"]),
                    "gateway_decrypt_p95_ms": _safe_pct(group["elapsed_ms"], 95),
                    "gateway_avg_overhead_bytes": _safe_mean(group["overhead_bytes"]),
    ...
    fog_rounds = (
        fog_df.groupby(["attempt_id", "device_suffix", "global_round"], dropna=False)
        .agg(
            fog_accuracy=("accuracy", "first"),
            fog_loss=("loss", "first"),
            fog_num_samples=("num_samples", "first"),
            fog_peer_count=("peer_count", "first"),
            fog_timestamp=("timestamp_dt", "first"),
        )
        .reset_index()
    )
            ...
            lambda group: pd.Series(
                {
                    ...
                    "server_avg_overhead_bytes": _safe_mean(group["overhead_bytes"]),
                }
            )
        )
        .reset_index()
    ...
    for _, row in global_history.sort_values(["attempt_id", "round"]).iterrows():
        ...
        for gateway_id in sorted(local_df["device_suffix"].dropna().unique().tolist()):
            ...
            if not gateway_row.empty:
                ...
                base[f"{gateway_id}_gateway_decrypt_avg_ms"] = gateway_row["gateway_decrypt_avg_ms"].iloc[0]
                base[f"{gateway_id}_gateway_decrypt_p95_ms"] = gateway_row["gateway_decrypt_p95_ms"].iloc[0]
                base[f"{gateway_id}_gateway_avg_overhead_bytes"] = gateway_row["gateway_avg_overhead_bytes"].iloc[0]
                base[f"{gateway_id}_gateway_samples_seen"] = gateway_row["gateway_samples_seen"].iloc[0]

            fog_row = fog_rounds.loc[
                (fog_rounds["attempt_id"] == attempt_id)
                & (fog_rounds["device_suffix"] == gateway_id)
                & (fog_rounds["global_round"] == global_round)
            ]
            if not fog_row.empty:
                base[f"{gateway_id}_fog_accuracy"] = fog_row["fog_accuracy"].iloc[0]
                base[f"{gateway_id}_fog_loss"] = fog_row["fog_loss"].iloc[0]
                base[f"{gateway_id}_fog_num_samples"] = fog_row["fog_num_samples"].iloc[0]
                base[f"{gateway_id}_fog_peer_count"] = fog_row["fog_peer_count"].iloc[0]
                base[f"{gateway_id}_fog_timestamp"] = fog_row["fog_timestamp"].iloc[0]

        server_dec = decrypt_server.loc[
            (decrypt_server["attempt_id"] == attempt_id) & (decrypt_server["global_round"] == global_round)
        ]
        if not server_dec.empty:
            for col in [
                ...
                "server_rpi_pc_avg_overhead_bytes",
            ]:
                base[col] = server_dec[col].iloc[0]

        server_enc = encrypt_server.loc[
            (encrypt_server["attempt_id"] == attempt_id) & (encrypt_server["global_round"] == global_round)
        ]
        if not server_enc.empty:
            for col in [
                ...
                "server_pc_rpi_avg_overhead_bytes",
            ]:
...
def compute_executive_summary(
    ...
    fog_aggregation_summary: pd.DataFrame,
    ...
    return {
        ...
        "avg_gateway_transport_p95_ms": _safe_mean(
            transport_summary.loc[
                ...
                & (transport_summary["operation"] == "decrypt"),
                "p95_ms",
            ]
        ),
        "avg_server_decrypt_p95_ms": _safe_mean(
            transport_summary.loc[
        ...
        "avg_payload_overhead_bytes": _safe_mean(transport_summary["avg_overhead_bytes"]),
        ...
        "avg_fog_accuracy": _safe_mean(fog_aggregation_summary["avg_accuracy"]),
        "avg_fog_loss": _safe_mean(fog_aggregation_summary["avg_loss"]),
...
def generate_plots(
    ...
    if not transport_summary.empty:
        ...
        ax.set_title("Latency p95 promedio por canal y operacion (CNN_FOG #2-#8)")
        ax.set_xlabel("Canal")
        ...
        fig, ax = plt.subplots(figsize=(10, 5))
        overhead = transport_summary.groupby(["source_scope", "channel"])["avg_overhead_bytes"].mean().reset_index()
        overhead["label"] = overhead["source_scope"] + " | " + overhead["channel"]
        ax.bar(overhead["label"], overhead["avg_overhead_bytes"], color="#ff7f0e")
        ax.set_title("Overhead promedio de payload por canal")
        ax.set_xlabel("Canal")
        ax.set_ylabel("Overhead promedio (bytes)")
        ax.tick_params(axis="x", rotation=45)
        path = output_dir / "transport_overhead_bytes.png"
        _save_plot(fig, path)
        plot_paths["transport_overhead_bytes"] = str(path)
    ...
    if not global_history_enriched.empty:
        ...
        ax1.set_title("Convergencia global promedio (CNN_FOG #2-#8)")
    ...
    if not fog_aggregation_summary.empty:
        fig, ax = plt.subplots(figsize=(11, 5))
        fog_curve = fog_aggregation_summary.groupby("device_suffix")[["avg_accuracy", "avg_loss"]].mean().reset_index()
        ax.bar(fog_curve["device_suffix"], fog_curve["avg_accuracy"], color="#17becf", label="Avg fog accuracy")
        ax.set_title("Accuracy promedio del pre-FedAvg Fog")
        ax.set_xlabel("Leader gateway")
        ax.set_ylabel("Accuracy")
        path = output_dir / "fog_preaggregation_accuracy.png"
        _save_plot(fig, path)
        plot_paths["fog_preaggregation_accuracy"] = str(path)
...
def write_spec_markdown(
    ...
    lines = [
        ...
        f"- p95 promedio ESP32->RPi decrypt: `{executive_summary['avg_gateway_transport_p95_ms']:.3f} ms`",
        f"- p95 promedio RPi->PC decrypt: `{executive_summary['avg_server_decrypt_p95_ms']:.3f} ms`",
        f"- Overhead promedio de payload: `{executive_summary['avg_payload_overhead_bytes']:.3f} bytes`",
        ...
        f"- Accuracy promedio del pre-FedAvg Fog: `{executive_summary['avg_fog_accuracy']:.4f}`",
        f"- Loss promedio del pre-FedAvg Fog: `{executive_summary['avg_fog_loss']:.4f}`",
...
def run_analysis(
    ...
    fog_aggregation_summary = compute_fog_aggregation_summary(data["model_metrics"])
    ...
    tables = {
        ...
        "fog_aggregation_sli_summary": fog_aggregation_summary,
...
if __name__ == "__main__":
    default_results_root = Path(
        r"C:\Users\VivoBook\Downloads\microproyecto2\microproyecto 2 porque wtf\src\hfl_v7-CNN\Results_FOG"
    )
    default_output_root = Path(
        r"C:\Users\VivoBook\Downloads\microproyecto2\microproyecto 2 porque wtf\src\Analisis de Modelos\RN_FOG\analysis_outputs"
    )
    bundle = run_analysis(

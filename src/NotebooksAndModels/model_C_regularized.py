"""
=============================================================================
 model_C_regularized.py — MLP REGULARIZADO: 128 → 64 → 32 → 6
 Hipótesis: L2 regularization + BatchNorm + alto dropout evita que el modelo
 memorice patrones de ataque y mejora la generalización para tráfico benigno.
 También usa FOCAL LOSS para clases difíciles (como benign).
=============================================================================
"""
...
MODEL_NAME = "Model_C_Regularized_128_64_32"

DATA_DIR = Path("/home/sajaimesp/Tesis_Sistemas/Data_Sets/Proyecto_Grado_DataSets")
EXPORT_DIR = Path("export_modelC"); EXPORT_DIR.mkdir(exist_ok=True)
...
# ====================== MODELO C: REGULARIZADO 128->64->32->6 ============
print(f"\n{'='*50}\n {MODEL_NAME}\n MLP: {X_tr.shape[1]} -> 128(L2+BN) -> 64(L2+BN) -> 32(L2+BN) -> {num_classes}\n{'='*50}")
...
x = tf.keras.layers.Dense(128, kernel_regularizer=tf.keras.regularizers.l2(L2_REG))(x)
...
x = tf.keras.layers.Dense(32, kernel_regularizer=tf.keras.regularizers.l2(L2_REG))(x)
x = tf.keras.layers.BatchNormalization()(x)
x = tf.keras.layers.Activation("relu")(x)
...
# Focal Loss para mejorar clasificación de clases difíciles (benign)
...
# Learning rate scheduler: reduce al platear
lr_sched = tf.keras.callbacks.ReduceLROnPlateau(monitor="val_loss", factor=0.5, patience=4, min_lr=1e-6, verbose=1)

hist = model.fit(X_tr, y_tr, validation_data=(X_val,y_val), epochs=100, batch_size=512,
                 callbacks=[

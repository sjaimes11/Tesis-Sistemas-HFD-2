...
% Below journals below will use APA reference format:
...
% submit
%----------
% The class option "submit" will be changed to "accept" by the Editorial Office when the paper is accepted. This will only change the front page (e.g., the journal logo will become visible), the headings, and the copyright information. Line numbering will also be removed. Journal info and pagination for accepted papers will also be assigned by the Editorial Office.
...
%\dataset{DOI number or link to the deposited data set if the data set is published separately. If the data set will be published as a supplement to this paper, the journal editors will fill in this field. In this case, please submit the data set as a supplement.}
...
%
%\noindent This is an optional section, the goal of which is to increase the discoverability and readability of the article via search engines and other scholars. Highlights should not copy the abstract, but provide a simple overview that lets readers quickly find what the article is about and what can be cited. Each of these parts should be up to 2~bullet points.\vspace{3pt}\\
...
%
% TODO-REF: Buscar un trabajo de revisión reciente que aborde explícitamente las métricas de evaluación para TinyML (accuracy, latency, memory footprint y energy consumption).

No obstante, la capacidad de realizar inferencia local constituye únicamente una parte del ciclo de vida de un IDS inteligente. En escenarios operativos, los modelos deben actualizarse para incorporar nuevos patrones de tráfico, adaptarse a cambios en el comportamiento de la red y responder a amenazas emergentes. Sin embargo, la mayor parte de las investigaciones continúa concentrándose en la inferencia sobre dispositivos Edge, mientras que el entrenamiento y la actualización de los modelos suelen permanecer centralizados o realizarse en plataformas con recursos considerablemente superiores a los disponibles en microcontroladores \cite{Ficco2024,Ramadan2025}.
%
% TODO-REF: Incorporar una referencia reciente que analice específicamente el aprendizaje continuo (continual learning o on-device adaptation) aplicado a TinyML en IoT.
...
%
% TODO-REF: Incorporar una referencia reciente sobre ataques de inferencia, poisoning de modelos o privacidad de los gradientes en el aprendizaje federado aplicado a IoT.
...
%
% TODO-REF:
% Incorporar referencias recientes sobre evaluaciones comparativas entre Federated Learning, Hierarchical Federated Learning y esquemas protegidos mediante criptografía ligera.
...
La evaluación experimental se realizó siguiendo un protocolo común de aprendizaje federado jerárquico para todos los escenarios considerados en este trabajo. Independientemente del modelo TinyML, de la arquitectura de aprendizaje o del mecanismo de protección de las comunicaciones evaluado, todas las configuraciones experimentales compartieron exactamente la misma secuencia de ejecución, lo que garantiza que las diferencias observadas en los resultados sean consecuencia exclusiva de las variables experimentales definidas en la estrategia de evaluación. La Figura~\ref{fig:experimental_workflow} resume las tres fases que componen cada ronda del protocolo experimental.
...
Modelo de inferencia TinyML &
Comparación entre los modelos MLP y CNN-1D para analizar el compromiso entre la capacidad de detección y los los requisitos computacionales en dispositivos \textit{Edge}. \\
...
Desempeño del despliegue en dispositivos \textit{Edge} & RQ3 & Latencia de inferencia, utilización de CPU y consumo de memoria. \\
...
Para caracterizar este comportamiento se definieron dos dimensiones complementarias de evaluación. La primera, \textit{Cryptographic Performance}, analiza el desempeño temporal del mecanismo de protección mediante el tiempo de cifrado (\textit{Encryption Time}) y el de descifrado (\textit{Decryption Time}), lo que permite cuantificar el retardo introducido por las operaciones criptográficas. La segunda, \textit{Computational Overhead}, evalúa el impacto de dichas operaciones sobre los recursos del sistema utilizando la utilización del procesador (\textit{CPU Utilization}) y el consumo de memoria (\textit{Memory Utilization}) como indicadores del costo computacional asociado al mecanismo de seguridad.

En conjunto, estas dos dimensiones proporcionan una caracterización integral del desempeño de las comunicaciones seguras al evaluar tanto el costo temporal de las operaciones criptográficas como su impacto en los recursos computacionales disponibles. La combinación de estos indicadores permite determinar la viabilidad de incorporar mecanismos de comunicación segura en arquitecturas IoT distribuidas preservando un desempeño compatible con sus restricciones operacionales.
...
\\
\midrule
E1 & MLP & Edge--Cloud      & ASCON & TODO & TODO & TODO & TODO \\
E2 & MLP & Edge--Cloud      & PLAIN & TODO & TODO & TODO & TODO \\
E3 & MLP & Edge--Fog--Cloud & ASCON & TODO & TODO & TODO & TODO \\
E4 & MLP & Edge--Fog--Cloud & PLAIN & TODO & TODO & TODO & TODO \\
E5 & CNN & Edge--Cloud      & ASCON & TODO & TODO & TODO & TODO \\
E6 & CNN & Edge--Cloud      & PLAIN & TODO & TODO & TODO & TODO \\
E7 & CNN & Edge--Fog--Cloud & ASCON & TODO & TODO & TODO & TODO \\
E8 & CNN & Edge--Fog--Cloud & PLAIN & TODO & TODO & TODO & TODO \\
\bottomrule
\end{tabular}
\end{table*}
...
\begin{table*}[ht]
\centering
\caption{RESULTADOS PRELIMINARES SOLO CALCULADOS Detection performance obtained for the eight experimental configurations evaluated in the proposed architecture.}
\label{tab:detection_capability}
\footnotesize
\begin{tabular}{lccccccc}
\toprule
...
Más allá del efecto individual de cada decisión de diseño, el aspecto más relevante corresponde al comportamiento global de la arquitectura. **TODO:** Incorporar el rango de variación observado en las métricas de detección entre las ocho configuraciones experimentales y demostrar que dichas variaciones permanecen dentro de límites reducidos. Esta evidencia permitirá concluir que la capacidad de detección constituye una propiedad robusta de la arquitectura propuesta y no depende de una combinación particular del modelo de inferencia, de la estrategia de agregación o del mecanismo de protección utilizado.
...
\begin{figure*}[ht]
    \centering
    \includegraphics[width=0\textwidth]{Figures/Round_Duration_English.png}
...
\\
\midrule
Hajj et al.            & \checkmark & --          & \checkmark & --          & --          & --          \\
Chaurasia et al.       & \checkmark & --          & \checkmark & --          & --          & --          \\
Ficco et al.           & \checkmark & \checkmark  & \checkmark & --          & --          & \checkmark  \\
Roy et al.             & \checkmark & --          & \checkmark & --          & --          & --          \\
Tran et al.            & --          & \checkmark  & \checkmark & \checkmark  & \checkmark  & \checkmark  \\
Heydari \& Mahmoud     & \checkmark & \checkmark  & --          & --          & --          & \checkmark  \\
Saravana Balaji et al. & \checkmark & \checkmark  & --          & --          & \checkmark  & \checkmark  \\
\midrule
...
Los resultados obtenidos muestran que el comportamiento de este tipo de arquitecturas no depende exclusivamente del desempeño alcanzado por cada uno de sus componentes, sino de la interacción entre las decisiones de inferencia, aprendizaje y protección de las comunicaciones. La evidencia experimental indica que considerar conjuntamente estas dimensiones permite mantener un desempeño equilibrado bajo las restricciones propias de los entornos IoT, proporcionando una perspectiva arquitectónica para el diseño de sistemas distribuidos de detección de intrusiones.
...
\bibitem{CICIoT2023}
Neto, E. C., Dadkhah, S., Ferreira, R., Zohourian, A., Lu, R., Ghorbani, A. A., and others. CICIoT2023: A Real-Time Dataset and Benchmark for Large-Scale Attacks in IoT Environment. \emph{Sensors}, 23(13):5941, 2023.

\bibitem{CICFlowMeter}
Lashkari, A. H., Gil, G. D., Mamun, M. S. I., and Ghorbani, A. A. Characterization of Tor Traffic Using Time-Based Features. In \emph{Proceedings of the 3rd International Conference on Information Systems Security and Privacy (ICISSP)}, pages 253--262, 2017.

\bibitem{Zouhri2024}
Zouhri, H., Idri, A., and Ratnani, A. Evaluating the Impact of Filter-Based Feature Selection in Intrusion Detection Systems. \emph{International Journal of Information Security}, 23:759--785, 2024.

\bibitem{Banbury2020}
Banbury, C. R., Reddi, V. J., Lam, M., Fu, W., Fazel, A., Holleman, J., and others. Benchmarking TinyML Systems: Challenges and Direction. arXiv:2003.04821, 2020.

\bibitem{Li2017}
Li, J., Cheng, K., Wang, S., Morstatter, F., Trevino, R. P., Tang, J., and Liu, H. Feature Selection: A Data Perspective. \emph{ACM Computing Surveys}, 50(6):94:1--94:45, 2018.

\bibitem{Ficco2024}
Ficco, M., Palmieri, F., and Castiglione, A. A Fog-Edge-Enabled Intrusion Detection System for Smart Grids. \emph{Journal of Cloud Computing}, 13:43, 2024. doi:10.1186/s13677-024-00609-9.

\bibitem{Hajj2023}
Hajj, S., Azar, J., Bou Abdo, J., Demerjian, J., Guyeux, C., Makhoul, A., and Ginhac, D. Cross-Layer Federated Learning for Lightweight IoT Intrusion Detection Systems. \emph{Sensors}, 23(16):7038, 2023. doi:10.3390/s23167038.

\bibitem{Moustafa2019}
Moustafa, N., Turnbull, B. P., and Choo, K.-K. R. An Ensemble Intrusion Detection Technique Based on Proposed Statistical Flow Features for Protecting Network Traffic of Internet of Things. \emph{IEEE Internet of Things Journal}, 6(3):4815--4830, 2019. doi:10.1109/JIOT.2018.2871719.

\bibitem{Warden2019}
Warden, P., and Situnayake, D. \emph{TinyML: Machine Learning with TensorFlow Lite on Arduino and Ultra-Low-Power Microcontrollers}. O'Reilly Media, Sebastopol, CA, USA, 2019.

\bibitem{Banbury2020}
Banbury, C. R., Reddi, V. J., Lam, M., Fu, W., Fazel, A., Holleman, J., Huang, X., Hurtado, R., Kanter, D., Lokhmotov, A., and others. TinyML Systems: Challenges and Direction. arXiv:2003.04821, 2020.

...
\begin{itemize}[noitemsep, topsep=0pt]
    ...
    \item Mantener un \textbf{buffer de entrenamiento} de $N_s = 30$ muestras etiquetadas mediante una función heurística basada en las distribuciones reales del dataset.
...
La operación de la arquitectura fue implementada en dos modos:
\begin{itemize}[noitemsep, topsep=0pt]
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{Kap3b/standard_vs_fog_sidebyside.png}
    \caption{Modos de operación: (a)~estándar con agregación directa PC–gateways y (b)~FOG con agregación jerárquica mediante un gateway líder.}
    \label{fig:standard_vs_fog}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\textwidth]{Kap3b/mlp_architecture_figure.png}
    \caption{Arquitectura del modelo MLP multiclase. Las capas ocultas 3 y de salida (recuadro punteado) conforman las \textit{capas federadas} ($W_3$, $b_3$, $W_4$, $b_4$), cuyos 163 parámetros son los únicos intercambiados durante el aprendizaje federado.}
...
\begin{enumerate}
    \item \textbf{Nivel Edge (ESP32 $\to$ RPi):} Los nodos ESP32 envían vectores de features al gateway cada 5 segundos vía MQTT (\texttt{fl/features}). El gateway acumula $N_s = 30$ muestras etiquetadas heurísticamente.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{images/Metodologia/hfl_v7_bidirectional_flow.png}

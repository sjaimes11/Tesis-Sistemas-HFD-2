...
// CORREGIDO: minimo de paquetes, deteccion hibrida, serial reducido
// =====================================================================
...
const char* STA_SSID = "TP-Link_AADB"; //"JAIMES_PUERTO 2.4";
const char* STA_PASS = "55707954"; //"Anderson123";
...
// Minimo de paquetes para que el modelo sea confiable
// Con menos de esto, las features son demasiado dispersas
constexpr uint32_t MIN_PKTS_FOR_ML = 10;

// Umbral de regla: mas de N paquetes en 5s es sospechoso sin importar el modelo
  ...
  bool onEvent(sMQTTEvent *event) override {
    switch (event->Type()) {
      case NewClient_sMQTTEventType: {
        brokerConnections++;
        brokerTrackEvent(64);
        // Solo imprimir cada 50 conexiones para no bloquear serial
...
void analyzeAndAlert(float features[FEATURE_COUNT], const char* source, bool ruleTriggered) {
  ...
  // Solo entrenar si hay suficientes paquetes para features confiables
  if ((uint32_t)features[0] >= MIN_PKTS_FOR_ML) {
    trainLocalStep(features, finalPred);
  ...
  if (millis() - brokerLastWindowMs >= BROKER_WINDOW_MS) {
    ...
    if (brokerGlobalPkts > 0) {
      ...
      if (brokerGlobalPkts >= MIN_PKTS_FOR_ML || ruleTriggered) {
        ...
        Serial.println(") -> insuficiente, clasificado NORMAL");
      }
      resetBrokerFlow();
    }
  }

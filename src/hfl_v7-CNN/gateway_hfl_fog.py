...
def fog_fedavg():
    ...
    # CNN-1D: dense_1 (16->8), dense_out (8->3)
...
def send_fog_aggregated_to_pc(Wd1, bd1, Wdo, bdo, num_samples, accuracy, loss):
    ...
    print(f"[FOG-LEADER] Enviando pesos CNN Fog-agregados al servidor PC...")
    try:
...
def handle_fog_global_received(payload):
    ...
    print(f"\n{'='*60}")
    print(f" [FOG-PEER CNN] MODELO GLOBAL RECIBIDO DEL LÍDER (Ronda {current_round})")
    print(f" ASCON: Descifrado y autenticado OK")
    print(f"{'='*60}")

    model.get_layer("dense_1").set_weights([Wd1, bd1])
    model.get_layer("dense_out").set_weights([Wdo, bdo])
...
class DeployModelHandler(BaseHTTPRequestHandler):
    def do_POST(self):
        ...
        if self.path == "/deploy-model":
            ...
            print(f"\n{'='*60}")
            print(f" [FOG-LEADER CNN] MODELO GLOBAL RECIBIDO DEL PC (Ronda {current_round})")
            print(f"{'='*60}")
...
def broadcast_model_to_esp32s(Wd1, bd1, Wdo, bdo):
    ...
    if mqtt_local and mqtt_local.is_connected():
        ...
        print(f"[MQTT-LOCAL] Modelo CNN cifrado publicado para ESP32s ({len(envelope_str)}B)")
...
def broadcast_model_to_fog_peers(Wd1, bd1, Wdo, bdo):
    ...
    if mqtt_fog and mqtt_fog.is_connected():
        ...
        print(f"[MQTT-FOG] Modelo CNN cifrado publicado para peers Fog ({len(envelope_str)}B)")
...
def train_local_model():
    ...
    print(f"\n[TRAIN CNN] Entrenando localmente con {len(X_cnn)} muestras...")
    ...
    Wd1, bd1 = model.get_layer("dense_1").get_weights()
    Wdo, bdo = model.get_layer("dense_out").get_weights()
    ...
    if FOG_ROLE == "leader":
        with fog_weights_lock:
            ...
            print(f"[FOG-LEADER] Pesos CNN propios almacenados en buffer Fog")
            check_fog_aggregation_ready()

    elif FOG_ROLE == "peer":
        send_weights_to_fog_leader(Wd1, bd1, Wdo, bdo, len(X_cnn), final_acc, final_loss)

    metrics.print_live_summary()


def send_weights_to_fog_leader(Wd1, bd1, Wdo, bdo, num_samples, accuracy, loss):
    """Peer: publica pesos CNN locales al broker Fog del líder via MQTT + ASCON."""
    payload = {
    ...
    if mqtt_fog and mqtt_fog.is_connected():
        ...
        print(f"[MQTT-FOG] Pesos CNN cifrados enviados al líder Fog ({len(envelope_str)}B)")
    else:

"""
=============================================================================
 ascon_metrics_gateway.py — Recolector ASCON para Raspberry Gateways
=============================================================================
 Variante enfocada en identificar claramente de qué Raspberry proviene cada
 CSV y qué intento de ejecución representa.

 Patrón de salida:
   Results/ascon_metrics_<device>_<suffix>_<n>.csv

 Ejemplos:
   Results/ascon_metrics_gateway_gateway_A_1.csv
   Results/ascon_metrics_gateway_gateway_B_2.csv
=============================================================================
"""
...
def _next_results_csv_path(device_name: str, suffix: str) -> Path:
    ...
    prefix = f"ascon_metrics_{_slug(device_name)}_{_slug(suffix)}_"
    indices = []
...
class AsconMetrics:
    ...
    def record(self, channel, operation, pt_size, enc_size, elapsed_ms, fl_round, **extra_fields):
        ...
        print(
            f"  [ASCON {tag}] {self.device_suffix} | {channel}: "
            f"{elapsed_ms:.3f}ms | {pt_size}B -> {enc_size}B (+{enc_size - pt_size}B)"
        )
    ...
    def get_summary_by_channel(self):
        ...
        for record in self.records:
            channel = channels[record["channel"]]
            if record["operation"] == "encrypt":
                channel["enc_times"].append(record["elapsed_ms"])
            else:
                channel["dec_times"].append(record["elapsed_ms"])
            channel["overheads"].append(record["overhead_bytes"])
            channel["pt_sizes"].append(record["pt_bytes"])

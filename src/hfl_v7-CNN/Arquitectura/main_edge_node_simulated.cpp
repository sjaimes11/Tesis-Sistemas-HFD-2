...
void onMqttGatewayCallback(char* topic, byte* payload, unsigned int length) {
  ...
  size_t ct_len=0, tlen=0, nlen=0;
  mbedtls_base64_decode(ct,   sizeof(ct), &ct_len, (unsigned char*)ct_b64.c_str(),    ct_b64.length());
  mbedtls_base64_decode(tag,  16,         &tlen,   (unsigned char*)tag_b64.c_str(),   tag_b64.length());
  mbedtls_base64_decode(nonce,16,         &nlen,   (unsigned char*)nonce_b64.c_str(), nonce_b64.length());
  ...
  if (!ok) { Serial.println("[ERROR] ASCON: Tag invalido."); return; }
